...
const seedDefaultHalls = require('./seedHalls');
const logger = require('./logger');

// Load env vars early
dotenv.config();

// Connect to database, then seed default halls on a fresh (empty) database
...
// Login-specific limiter: stricter, per-IP defense-in-depth on top of the
// per-account lockout in routes/auth.js — 20 attempts per 15 min per IP.
const loginLimiter = rateLimit({
  ...
  handler: (req, res) => {
    ...
    res.status(429).json({ message: 'Too many login attempts from this network. Please try again later.' });
  },
});

// ── 6. Request Logger Middleware ──────────────────────────────────────────────
// Failed requests (4xx/5xx) always log to error.log. Successful requests
// only log to success.log for state-changing methods (POST/PATCH/DELETE)
// — routine successful GETs (polling, page loads) are skipped so
// success.log stays a meaningful record of real actions instead of raw
// traffic noise.
app.use((req, res, next) => {
  ...
  res.on('finish', () => {
    ...
    const isStateChange = req.method !== 'GET';
    if (!isFailure && !isStateChange) return;
...
app.use('/api/hall-layouts', require('./routes/hallLayouts'));
