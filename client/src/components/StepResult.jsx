    ...
    <div className="step-content p-4 bg-white rounded-4 shadow-sm border" style={{ borderColor: '#E8E2E5' }}>
      ...
      <div style={{
        ...
        <p style={{ color: '#9B8F94', fontSize: '13px', margin: 0 }}>
          Hall plan generated. Print the hall plan.
        </p>
      </div>
      ...
      {/* ── Actions ── */}
      <div className="d-flex flex-column flex-sm-row justify-content-center gap-3 pt-3 border-top no-print" style={{ flexWrap: 'wrap' }}>
        {onModify && (
          <button
        ...
        {allocation._id && (
          <>
            <Link
              ...
              className="btn btn-outline-primary d-flex align-items-center justify-content-center gap-2 px-4 py-2 rounded-pill fw-bold"
            >
              <FiPrinter size={15} /> Print Hall Plan
            </Link>

            <button
              onClick={() => navigate('/seating-allotment', { state: { preSelectId: allocation._id } })}
              className="btn btn-primary d-flex align-items-center justify-content-center gap-2 px-5 py-2 rounded-pill fw-bold"
              style={{
                background: 'linear-gradient(135deg, #B42B6A 0%, #9A2259 100%)',
                color: 'white',
                boxShadow: '0 4px 12px rgba(180,43,106,0.25)',
                border: 'none'
              }}
            >
              <FiUsers size={15} /> Go to Seating Allotment
            </button>
          </>
        )}
      </div>
