...
/**
 * Separators that combine multiple subject names into one elective-group
 * header: / , | ; & +  — deliberately NOT "-", since a hyphen is often
 * part of a real subject name (UI-UX, Subject-A, Data-Science) rather
 * than a separator. Runs of separators/whitespace collapse to one split
 * point, so "AD / IP / / BDA" never produces empty subjects.
 */
const SUBJECT_SEPARATORS = /[\/,|;&+]+/;

/**
 * Split a single elective-group header into its canonical subject names.
 * The header is the sole source of truth — nothing is hard-coded. Works
 * for any number of subjects:
 *   "AD/IP/BDA"     → ["AD", "IP", "BDA"]
 *   "AD, IP, BDA"   → same (comma separator)
 *   "IOT"           → ["IOT"] (single-subject group, no separator)
 */
const splitSubjectNames = (header) => {
  return header.split(SUBJECT_SEPARATORS).map(part => part.trim()).filter(Boolean);
};
...
const buildColumnLookup = (header) => {
  const lookup = {};
  splitSubjectNames(header).forEach(canonical => {
    lookup[looseNorm(canonical)] = canonical;
  });
  return lookup;
};
...
const StepFileUpload = ({ formData, setFormData, onDataParsed }) => {
   ...
   * Known structural columns: Roll No, Register Number, Name of the
   * Student, Section. Every other non-empty header is treated as an
   * elective/subject group column — the Excel file is the sole source of
   * truth for subject names, not a hard-coded list. A separator (see
   * SUBJECT_SEPARATORS above) combines multiple subjects into one group;
   * a header with no separator is simply a single-subject group.
   */
  const classifyColumns = (headers) => {
    const result = {
      ...
      electiveGroups: []    // any other non-structural column, e.g. "AD/IP/BDA" or "IOT"
    };
    ...
    headers.forEach(h => {
      ...
      if (lower === 'roll no' || lower === 'rollno' || lower === 'roll no.' || lower === 's.no' || lower === 'sno' || lower === 'sl no' || lower === 's no') {
        ...
        // Anything else non-empty and non-structural is a subject/elective
        // group column, whether it names one subject or several.
      ...
      reader.onload = (e) => {
        try {
          ...
          if (columns.electiveGroups.length === 0) {
            reject(new Error('Could not find any subject/elective columns — every header matched a structural column (Register Number, Name, Section). Add at least one more column named after the subject(s), e.g. a single subject like "IOT" or multiple combined with a separator like "AD/IP/BDA". Found headers: ' + headers.join(', ')));
            return;
          }
          ...
          // If the file has BOTH a "Roll No" column and a separate "Register
          // Number" column, rollCol above prefers Register Number (the
          // identifier used for printing/display everywhere). The Roll No
          // column's values are captured separately here — used to order
          // both the Attendance Sheet and actual seat/bench assignment,
          // instead of Register Number (which isn't always sequential) or
          // raw Excel row order.
          const hasSeparateRollNoCol = columns.rollNo && columns.rollNo !== rollCol;
          ...
          jsonData.forEach(row => {
            ...
            const rollNo = hasSeparateRollNoCol ? String(row[columns.rollNo] || '').trim() : '';
            ...
            students.push({
              ...
              rollNo, // separate Roll No column, if present — drives seating + attendance order
            });
          });
  ...
  const processFile = useCallback(async (file) => {
    ...
    try {
      ...
      // e.g. "AD/IP/BDA" → ["AD", "IP", "BDA"]; "IOT" → ["IOT"]
      const electives = [];
      electiveGroups.forEach(header => {
        splitSubjectNames(header).forEach(canonical => {
          if (!electives.includes(canonical)) electives.push(canonical);
        });
      });
      ...
      {!selectedFile && (
        <div className="mt-4 p-3 rounded-3 border" style={{ backgroundColor: '#FAFAFA', borderColor: '#E8E2E5' }}>
          ...
          <div className="d-flex flex-column gap-1" style={{ fontSize: '12px' }}>
            <div className="d-flex flex-column flex-sm-row align-items-start gap-1 gap-sm-3">
            ...
            <div className="d-flex flex-column flex-sm-row align-items-start gap-1 gap-sm-3 mt-1 pt-1" style={{ borderTop: '1px dashed #E8E2E5' }}>
              <span className="mono fw-bold" style={{ color: '#B42B6A', minWidth: '160px' }}>Elective Columns</span>
              <span className="text-muted">Any other column name is a subject/elective group — e.g. a single subject like "IOT", or several combined with a separator ("/", ",", "|", ";", "&", "+") like "AD/IP/BDA"</span>
            </div>
          </div>
        </div>
      )}
