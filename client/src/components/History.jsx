...
import { FiEye, FiPrinter, FiTrash2, FiAlertTriangle, FiPlus, FiEdit3, FiX, FiSave, FiCalendar, FiClipboard, FiLayout, FiLayers } from 'react-icons/fi';
...
const History = () => {
  ...
  return (
    <div className="px-3 px-lg-4 py-4 page-enter">
      ...
      {/* Header */}
      <div className="d-flex justify-content-between align-items-center mb-4 pb-2 border-bottom page-header-wrap history-header-wrap" style={{ gap: '10px' }}>
      ...
      {/* ── Tabs ── */}
      <div className="history-tabs-mobile-wrap mb-4">
        <div className="history-tabs">
          ...
          <button
            className={`history-tab-btn${activeTab === 'seating' ? ' active' : ''}`}
            onClick={() => setActiveTab('seating')}
          >
            <FiLayers size={14} /> Seat Charts
          </button>
        </div>
      </div>
      ...
      {allocations.length === 0 ? (
        ...
        <div className="tab-content-enter" key={activeTab}>
          {/* ── Desktop Table ── */}
          <div className="d-none d-lg-block bg-white rounded-4 border shadow-sm overflow-hidden mb-4">
                ...
                {allocations.map(alloc => (
                  <tr key={alloc._id} className="history-row-hover stagger-item" style={{ transition:'all 0.18s ease' }}>
                    ...
                    <td className="px-4 py-3 text-center">
                      <div className="fw-bold" style={{ color:'#B42B6A', fontSize:'16px' }}>{alloc.totalStrength}</div>
                      {alloc.layoutBased ? (
                        <Link 
                          to={`/seat-chart/${alloc._id}`} 
                          style={{ 
                            fontSize:'9px', fontWeight:'700', background:'#F5F3FF', color:'#7C3AED', 
                            border:'1px solid rgba(124,58,237,0.2)', borderRadius:'50px', 
                            padding:'2px 7px', marginTop:'3px', display:'inline-block',
                        ...
                        <Link 
                          to="/seating-allotment" 
                          state={{ preSelectId: alloc._id }}
                          style={{ 
                            fontSize:'9px', fontWeight:'700', background:'#FEF2F2', color:'#EF4444', 
                            border:'1px solid rgba(239,68,68,0.2)', borderRadius:'50px', 
                            padding:'2px 7px', marginTop:'3px', display:'inline-block',
                            textDecoration: 'none', cursor: 'pointer', transition: 'all 0.15s'
                          }}
                      ...
                      <div className="d-flex justify-content-end gap-2">
                        ...
                        {activeTab === 'hallplan' ? (
                          <>
                            ...
                            {alloc.layoutBased && (
                              <Link to={`/seat-chart/${alloc._id}`} className="action-icon-btn aib-seat" title="View Seat Chart">
                                <FiLayers size={14} />
                              </Link>
                            )}
                          </>
                          ...
                          alloc.layoutBased ? (
                            ...
                            <Link to="/seating-allotment" state={{ preSelectId: alloc._id }} className="action-icon-btn aib-seat" title="Generate Seating">
                              <FiLayers size={14} />
                            </Link>
                          )
                        )}
          ...
          {/* ── Mobile Cards ── */}
          <div className="d-flex d-lg-none flex-column gap-3 mb-4 history-cards-mobile">
            {allocations.map((alloc, idx) => (
              <div key={alloc._id} className="bg-white p-4 rounded-4 border shadow-sm stagger-item history-card-mobile" style={{ animationDelay: `${idx * 60}ms` }}>
                <div className="d-flex justify-content-between align-items-center mb-2">
                  ...
                  <div style={{ display:'flex', gap:'5px', alignItems:'center' }}>
                    {alloc.layoutBased ? (
                      <Link to={`/seat-chart/${alloc._id}`} style={{ fontSize:'9px', fontWeight:'700', background:'#F5F3FF', color:'#7C3AED', border:'1px solid rgba(124,58,237,0.2)', borderRadius:'50px', padding:'2px 8px', textDecoration:'none' }}>
                        Seating Ready
                      </Link>
                    ) : (
                      <Link to="/seating-allotment" state={{ preSelectId: alloc._id }} style={{ fontSize:'9px', fontWeight:'700', background:'#FEF2F2', color:'#EF4444', border:'1px solid rgba(239,68,68,0.2)', borderRadius:'50px', padding:'2px 8px', textDecoration:'none' }}>
                        Allot Seats
                      </Link>
                    )}
                ...
                <div className="d-flex flex-wrap gap-2 pt-3 mt-3" style={{ borderTop:'1px dashed #E8E2E5' }}>
                  ...
                  {activeTab === 'hallplan' ? (
                    <>
                      ...
                      {alloc.layoutBased && (
                        <Link to={`/seat-chart/${alloc._id}`} className="action-mobile-btn aib-seat">
                          <FiLayers size={14} /> Seat Chart
                        </Link>
                      )}
                    </>
                  ) : activeTab === 'attendance' ? (
                    ...
                    alloc.layoutBased ? (
                      ...
                      <Link to="/seating-allotment" state={{ preSelectId: alloc._id }} className="action-mobile-btn aib-seat">
                        <FiLayers size={14} /> Generate
                      </Link>
                    )
                  )}
