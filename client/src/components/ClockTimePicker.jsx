...
const TimeSpinner = ({ value, min, max, step = 1, onChange, label }) => {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '2px' }}>
      ...
      <div className="time-spinner-value" style={{
...
const PeriodToggle = ({ value, onChange }) => (
  <div className="period-toggle" style={{ display: 'flex', flexDirection: 'column', gap: '5px', marginLeft: '4px', marginTop: '-14px' }}>
    {['AM', 'PM'].map(p => (
      <button
        key={p}
        type="button"
...
const ClockTimePicker = ({ session, value, onChange }) => {
  ...
  return (
    <div className="clock-time-picker" style={{
      ...
      <div className="clock-pickers-row" style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
        {/* Start time */}
        <div className="clock-time-section" style={sectionStyle}>
          <span style={sectionLabel}>Start Time</span>
          <div className="clock-box" style={clockBox}>
      ...
      <style>{`
        @media (max-width: 480px) {
          .clock-time-section { width: 100%; box-sizing: border-box; }
          .clock-box {
            width: 100% !important;
          ...
          .period-toggle { margin-left: 2px !important; }
          .period-toggle-btn { padding: 4px 6px !important; font-size: 9px !important; }
        }
        @media (max-width: 360px) {
          .clock-box { padding: 8px 4px !important; }
          .time-spinner-value { font-size: 18px !important; min-width: 36px !important; padding: 5px 6px !important; }
        }
      `}</style>
    </div>
  );
